...
Since last assignment, I've added some additional methods to add extra attributes to an item so users can associate things like ratings with an item.  I also decided to add a contains method to the kernel methods so that the preconditions of the other methods can be checked.  In addition, I changed the representation of barcodes from ints to strings since most barcodes are over the integer limit.  I considered using something like a NaturalNumber to do this, but there are benefits to being able to use any character in the item identifier.  There is also now an iterator.
...
This assignment was fun and I feel like I have a much better idea of what I want to do with my component now!
