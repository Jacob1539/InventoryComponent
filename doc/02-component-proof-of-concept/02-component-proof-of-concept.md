...
For this assignment, I'll be creating an Inventory class that stores InventoryItem objects.  The design i've chosen is to represent Inventory as a map<int, InventoryItem> where InventoryItem is a nested class.  We previously discussed using an InventoryItemBuilder class, but I believe that the nested class approach will work better because the building can be done as a direct interaction with the Inventory class.  This means that the user only needs to worry about one class.
